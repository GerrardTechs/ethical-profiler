"""
database.py — v4 with auth + analytics
Tables: users, sessions, scan_history, scan_details, username_cache, api_usage_log
"""
...
_ENV_FILE = _EnvPath(__file__).resolve().parent.parent.parent / ".env"
...
import sys as _sys
_default_db = (
    str(_EnvPath(__file__).resolve().parent.parent.parent / "data" / "profiler.db")
...
async def init_db() -> None:
    ...
    async with aiosqlite.connect(str(DB_PATH)) as db:
        ...
        if count == 0:
            ...
            await db.commit()
            logger.warning("[DB] Default admin created: admin / admin123 — SEGERA GANTI!")
